...
<?php
	...
	if(!isset($_SESSION['login']) && isset($_POST['username']) && isset($_POST['password']))
	{
		...
		$userRowAdmins = $dePoveziSe->prepare("SELECT * FROM admins WHERE username='".$username."' AND password='".$password."'");
		$userRowAdmins->execute();
		$resultAdmins = $userRowAdmins->fetch();

		$userRowClients = $dePoveziSe->prepare("SELECT * FROM online_klijenti WHERE username='".$username."' AND password='".$password."'");
